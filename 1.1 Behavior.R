...
behavior.overview %>% count(subject) %>% filter(n != 2)
behavior.overview %>% filter(noET %>% is.na() == F)
...
behavior = behavior %>% 
  ...
  mutate(trialCounter = trial, #make a copy and override original
         trial = if_else(trial < (block-1) * trials.N.block, trial %% trials.N.block + (block-1) * trials.N.block, trial)) %>% #correct trial count for missing blocks
...
behavior %>% summarize(.by = subject, trials = max(trialCounter)) %>% filter(trials!=trials.N) 
#note: don't use count because there are 3 rows per trial
...
behavior = behavior %>% 
  ...
  mutate(angry = case_when(distractor_left %>% grepl("category1", .) ~ "left",
         ...
         target = case_when(target_right %>% is.na() ~ "left",
                            target_left %>% is.na() ~ "right",
                            T ~ NA) %>% factor(levels = c("left", "right")), #left = first level
         congruency = if_else(angry == target, "angry", "neutral") %>% as_factor(),
         targetKind = case_when(target_right %>% is.na() ~ target_left,
  ...
  mutate(response = response %>% as.integer(),
         response = response - 1,
         keyAssign = case_when(paradigm == "Localization" ~ NA, #no key assignment for Localization
                               #only for discrimination:
                               subject %>% gsub("\\D", "", .) %>% as.integer() %>% {. %% 2} == 1 ~ 1, #subject number odd: 1
                               T ~ 2), #subject number even: 2
         response = case_when(paradigm == "Discrimination" & keyAssign == 1 ~ 
                                if_else(response==1, ":", "..") == targetKind,
                              paradigm == "Discrimination" & keyAssign == 2 ~ 
                                if_else(response==1, "..", ":") == targetKind,
                              paradigm == "Localization" ~
                                response == 1 & target == "left" | response == 2 & target == "right")
  ) %>%
...
sequences = sequences %>% separate(subject, c("subject", "block")) %>% 
  ...
  mutate(paradigm = case_when(subject %>% str_starts("a") ~ "Discrimination",
                              subject %>% str_starts("b") ~ "Localization",
                              T ~ NA) %>% as_factor(),
...
#wrong responses
behavior %>% summarize(.by = c(subject, paradigm), wrong_n = sum(response == FALSE, na.rm=T), `wrong_%` = mean(response == FALSE, na.rm=T)) %>% summarize(.by = paradigm, across(starts_with("wrong_"), list(m = mean, sd = sd)))

#=> unusable trials
behavior %>% summarize(.by = c(subject, paradigm), 
                       missing_n = sum(response %>% is.na() | expositionCheck %>% is.na() | response == F),
                       `missing_%` = mean(response %>% is.na() | expositionCheck %>% is.na() | response == F)) %>% 
...
behavior.valid = behavior %>% filter(response %>% is.na() == F,
                                     ...
                                     response != F) #kick out incorrect responses
...
behavior.reliability %>% pivot_wider(names_from = c(congruency, split), values_from = rt) %>% 
  summarize(.by = c(paradigm, SOA),
            ...
            rel_sb = cor(neutral_odd - angry_odd, neutral_even - angry_even) %>% spearmanBrown()) %>% arrange(desc(rel_sb))
...
behavior.reliability %>% pivot_wider(names_from = split, values_from = rt) %>% 
  summarize(.by = c(paradigm, SOA, congruency),
            ...
            rel_sb = cor(odd, even) %>% spearmanBrown()) %>% arrange(desc(rel_sb))
